...
export default function CategoryManager() {
  ...
  const [formData, setFormData] = useState({ name: '', imageUrl: '', subText: '' });
  ...
  const handleOpenModal = (category?: Category) => {
    if (category) {
      ...
      setFormData({ name: category.name, imageUrl: category.imageUrl, subText: category.subText || '' });
    } else {
      setEditingCategory(null);
      setFormData({ name: '', imageUrl: '', subText: '' });
    }
    setError('');
    setIsModalOpen(true);
  };
